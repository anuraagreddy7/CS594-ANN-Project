...
def run_all_experiments():
    ...
    print(
        "Relation between spread and height: The height of the tree is approximately O(log Δ), where Δ is the spread.")
    ...
    plt.xlabel('ε')
    plt.ylabel('Average Distance')
    plt.title('Query point (500,500) after third deletion')
    plt.grid(True)
    plt.savefig('q0_after_3rd_deletion.png')
    plt.close()
    ...
    plt.xlabel('ε')
    plt.ylabel('Average Distance')
    plt.title('Query point (1000,1000) after third deletion')
    plt.grid(True)
    plt.savefig('q1_after_3rd_deletion.png')
    plt.close()
...
def run_insertion_experiments():
    ...
    avg_time_1, avg_dist_1 = run_random_queries(quadtree, (1000, 1000), (1500, 1500))
    print(f"Average query time: {avg_time_1:.6f} seconds")
    print(f"Average distance: {avg_dist_1:.6f}")
